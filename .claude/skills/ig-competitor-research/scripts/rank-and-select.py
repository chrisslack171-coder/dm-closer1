...
<dataset.json>  the `get-actor-output` result the MCP tool saved to disk (the
...
<date_root>     /tmp/ig-research/<DATE>/
...
keeps the top N per handle, pools, sorts the whole set high-to-low, and writes:
  ...
  <date_root>/<rank>_<handle>/videourl.txt    reels/videos  (the muxed videoUrl)
...
def main():
    ...
    ap.add_argument("--expect-count", type=int, default=0,
                    help="hard-fail unless the dataset holds exactly this many items "
                         "(guards the get-actor-output-to-disk write against a dropped record)")
    args = ap.parse_args()
    ...
    if args.expect_count and len(items) != args.expect_count:
        print(f"[error] dataset.json holds {len(items)} items but --expect-count={args.expect_count}. "
              f"A record was dropped or garbled writing the get-actor-output result to disk — "
    ...
    if expected:
        items = [it for it in items if (it.get("ownerUsername") or "unknown") in expected]
        if not items:
