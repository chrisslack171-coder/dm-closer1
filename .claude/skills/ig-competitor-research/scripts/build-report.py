...
usage:
    uv run --with pillow python build-report.py <date_root | manifest.json> <out.html> [--open]
...
"""
import sys, os, json, glob, base64, html, io, subprocess
...
.transcript p{padding:0 0 14px;color:#c3c3cf;font-size:14.5px}
...
def assemble_from_dateroot(date_root):
    ...
    with open(os.path.join(date_root, "manifest.json"), "w") as f:
