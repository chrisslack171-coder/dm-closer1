...
- **Phase 1 — Scrape & prep:** one paid Apify call → pull the dataset to disk via the MCP (`get-actor-output`) → `rank-and-select.py` ranks the posts, scores each one's **breakout** (likes vs its creator's weekly median), picks the winners, and writes `selection.json` + scatters each post's media URL into its job dir.
...
1. **Resolve handles.** Read `competitor-list.md`, find `## Instagram`, extract handles from the `instagram.com/handle/` URLs, and **default to the first 5** (order = priority — don't ask). Inline handles override the default; "all" uses every handle. If the section is empty and no inline handles, ask.
...
3. **Run the actor (ONE paid call, all handles).** Load the Apify tool via `ToolSearch` → `select:mcp__Apify__call-actor`. Call `mcp__Apify__call-actor` with `actor: "apify/instagram-post-scraper"`, `previewOutput: false`, and this exact input, substituting the resolved handles into `username`:
   ...
   **Fire it async — never synchronously.** With `async` unset, `call-actor` blocks until every handle finishes scraping; a 5-handle `detailedData` run takes a few minutes — longer than the MCP request timeout — so it dies with `MCP error -32001: Request timed out` even though the scrape itself is healthy. **Always pass `async: true`**: the call returns instantly with a `runId` (status `READY`), so there's nothing to time out. The async response carries **no `datasetId`** — you get that from the finished run in the next breath. **This is the only paid call.** `detailedData` returns direct media for every format (reel `videoUrl`, carousel `images[]`), so this one scrape covers both ranking AND downloads. (~$0.003/post; ~$0.11 for 3 handles, ~$0.18 for 5.)

   **Then poll until it finishes.** Load the run-status tool: `ToolSearch` → `select:mcp__Apify__get-actor-run`. The scrape isn't instant, so don't pull immediately — wait, then check. Foreground `sleep` is blocked, so space each poll with a background wait: run `sleep 60` via Bash `run_in_background` (the harness re-invokes you when it exits), then call `mcp__Apify__get-actor-run` once with the `runId`. Repeat until `status` is `SUCCEEDED`, then **capture `datasetId` from that response** — that's what the next step pulls. If `status` comes back `FAILED`, `ABORTED`, or `TIMED-OUT`, stop and report the run — never pull a dead run. (A 5-handle run typically clears in 1–3 polls.)
4. **Pull the dataset to disk via the MCP.** Load the tool: `ToolSearch` → `select:mcp__Apify__get-actor-output`. Then call `mcp__Apify__get-actor-output` with **only** these two args — **no `fields` filter**:
   ...
   This rides the same OAuth that ran the paid scrape — **no token, ever: never look for an `APIFY_TOKEN` / API key and never ask the user to paste or `export` one; this skill has no key-based code path.**

   **Omitting `fields` is what forces the result onto disk — do not add it back.** A field-trimmed pull is small enough that the harness hands it back *inline* (in your context) on a light week, which would force you to hand-retype 1,000-char signed URLs (a mistyped char silently breaks the download — `--expect-count` only catches a *dropped* record, not a *mangled* one). Pulling the **full** records (~12K chars/post) reliably blows past the tool-result token cap, so the harness auto-spills the *entire* result to `…/<session-id>/tool-results/mcp-Apify-get-actor-output-<ts>.txt` and returns that path instead of the blob. **Never read the spill into context.** Extract the items straight to disk with `jq` (zero tokens spent on the URLs/captions):

   ```bash
   jq '.items' "<SPILLED_FILE>" > "<RUN_DIR>/dataset.json"
   ```

   `.items` is the bare array step 5 wants; `<N>` = `jq -r '.itemCount' "<SPILLED_FILE>"`. **No handle-filtering here** — `rank-and-select.py` is authoritative on the `--expect` roster and drops any stray handle the actor slips in (a tagged/collab post), so `dataset.json` holds the full pull verbatim and `--expect-count <N>` still guards the whole write.

   **If it *still* returns inline** (only on a very light week — a handful of total posts): do **not** hand-retype URLs. Re-call `get-actor-output` once with the same args to force the spill. Only if a genuinely tiny pull refuses to overflow, write the few inline records verbatim to `<RUN_DIR>/dataset.json` with the Write tool, then confirm `jq -r '.[] | (.videoUrl // empty), (.images[]? // empty)' "<RUN_DIR>/dataset.json" | grep -vc '^https'` prints `0` (every media URL intact) before continuing.
5. **Rank, score & prep (ONE script — no in-head sorting).** Run:
...
2. **Assemble + render + open (ONE command).** Run:
   ...
   ```
   uv run --quiet --with pillow python <SKILL_DIR>/scripts/build-report.py "<RUN_DIR>" "<project_root>/research/IG-Competitor-Research_<RUN>.html" --open
   ```
...
## Rules of Thumb
- **ONE paid Apify call (async), then pull the dataset via the MCP.** Fire `call-actor` at `detailedData` with `async: true` and poll `get-actor-run` until `SUCCEEDED` to get the `datasetId` — a synchronous call blocks past the MCP request timeout and dies with `-32001`. That's the only paid step → `get-actor-output` (same OAuth, no token) returns the items → you write them to `<RUN_DIR>/dataset.json` → `rank-and-select.py` does the rest. The dataset pull is MCP-only; never use an API token. No second scrape, no Chrome MCP, no Playwright.
...
- **Reels → `reel-breakdown.sh "<jobdir>" "$(cat <jobdir>/videourl.txt)"`** (curl the muxed `videoUrl` + ffmpeg keyframes + faster-whisper `small.en`). **Carousels/images → `fetch-images.sh "<jobdir>" $(cat <jobdir>/imageurls.txt)`**. Whisper override: `WHISPER_MODEL=medium.en`.
...
- **Output is a self-contained HTML report** built by `build-report.py` (Pillow downscales + base64-embeds every slide/frame — portable, no asset folder, no expiring links). Swipeable carousels, reel keyframe galleries, collapsible transcripts, breakout badges, "View on Instagram" buttons, 🔬 favicon. `--open` pops it in the browser.
