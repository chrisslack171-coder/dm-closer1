...
**Pop from where he points.** The signature beat: a glass panel slides in from the **side his hand gestures to**
(left or right), holds while he references it, exits before the next line. Slide from `x:±60`, `power3.out`;
exit `opacity:0` + hard-kill. Default to **visual** panels (bars/diagram/icons/checks), not text walls
(reserve text cards for hook/punchline) — see [[graphics-visual-not-text-heavy]].
...
for an optional off-ramp. Pin it in the **takeover** (face cards right, rail fills the left). It naturally reads
...
> `/tmp` and got wiped by macOS's overnight `/tmp` clear — keep the source durable; only `renders/` + base slices
> belong in `/tmp`. See [`../workflows/incremental-graphics.md`](../workflows/incremental-graphics.md).
