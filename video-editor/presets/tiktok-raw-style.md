...
- **Always on:** captions run from the first word to the last, the whole video. The hook card just overlays
  on top during its window — it does NOT replace the captions underneath. See [[captions-always-on]].
...
Every word runs through [`caption-corrections.json`](caption-corrections.json) (`auto` map) plus the builder's
niche fixes — `lark → larp`, `school → YourBrand`, `hurk → YourProduct`, casing for `YourName`/`YourApp`/`GitHub` —
before captioning (display text only; the source transcript is never touched). The builder also **drops
