...
**d. The idea (`content`).** Write the actual creative direction, not a category — what's on screen, the hierarchy, the hero element. **Describe the motion** — what animates and in what order — since the graphics run visual-heavy and a still description isn't enough to build from. Concrete enough that the creation step can build it without guessing. For **short-explainer**, every graphic obeys `presets/signature-style.md` (locked colors/fonts/spotlight bg) — reference it; design each fresh, don't stamp.
...
- **short-tiktok** — **exactly one graphic**: the front hook card. Default everything else plain. A second graphic only if a single stat is too big to skip.
...
Report back tight: total beats, how many got graphics (and the % for short-form), and the 2–3 strongest graphic ideas. Flag any beat you're unsure on with `⚠️` so you can call it.
...
```json
{
  ...
  "duration": 136.56,
  "preset": "presets/signature-style.md",
  "beats": [
    {
...
The plan feeds **step 3b (creation)** — the HyperFrames skills (`general-video`/`hyperframes`, `motion-graphics`, `graphic-overlays`) build each `graphic:true` beat at its `start`/`end` in its `region`, per the `content` idea. For short-explainer, build to `presets/signature-style.md`. Then the **second pass** adjusts, and **captions** (short-form) come after.
